...
import {PayoutMathV3} from "./libraries/PayoutMathV3.sol";
import {RollResolutionV3} from "./libraries/RollResolutionV3.sol";
...
contract CrapsGameV3 is ICrapsGameV3, VRFConsumerBaseV2Plus, Pausable, ReentrancyGuard {
    ...
    function _startRoll(address player) internal returns (uint256 requestId) {
        ...
        if (!RollResolutionV3.hasRollableBet(activeBets)) revert InvalidAmount(0);

        uint256 worstCase = PayoutMathV3.maxPossiblePayout(activeBets, session.point, LAY_WIN_VIG_BPS);
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] calldata randomWords) internal override {
        ...
        uint8 nextPoint;
        uint8 nextPhase;
        (session.bets, returnedToAvailable, lostToBankroll, payout, nextPoint, nextPhase) = RollResolutionV3.resolveRoll(
            session.bets,
            priorPoint,
            sum,
            die1,
