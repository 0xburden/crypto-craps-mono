...
function resolveReportPath(explicitPath: string | undefined): string | undefined {
  ...
  if (network.name === "baseSepolia") {
    return path.join("deployments", "sepolia-findings.md");
  }
...
async function main() {
  ...
  const deploymentPath = args.deployment ?? process.env.CRAPS_SMOKE_DEPLOYMENT ?? getDefaultDeploymentArtifactPath(network.name);
  const reportPath = resolveReportPath(args.report ?? process.env.CRAPS_SMOKE_REPORT);
  ...
  const game = await ethers.getContractAt("CrapsGame", deployment.contractAddress, signer);
  ...
  if (initialState.paused) {
    throw new Error("CrapsGame is paused; smoke test cannot proceed");
  }
  ...
  let depositNote = `deposit(100e6-equivalent) updated available balance by ${expectedAvailableDelta} and accrued fees by ${expectedFee}`;
  ...
  if (Number(workingState.phase) !== 1) {
    const openReceipt = await (await game.openSession()).wait();
    txHashes.openSession = openReceipt!.hash;
    workingState = await waitForCondition(
      () => game.getPlayerState(player),
      (state) => Number(state.phase) === 1,
      timeoutMs,
      pollIntervalMs,
      "session open"
    );
  ...
  if (workingState.bets.passLine.amount !== passLineBet) {
    const placeReceipt = await (await game.placeBet(PASS_LINE, passLineBet)).wait();
    txHashes.placeBet = placeReceipt!.hash;
    workingState = await waitForCondition(
      () => game.getPlayerState(player),
      (state) => state.bets.passLine.amount === passLineBet,
      timeoutMs,
      pollIntervalMs,
      "pass line bet placement"
    );
  }

  const rollRequestedBlock = await ethers.provider.getBlockNumber();
  const rollReceipt = await (await game.rollDice()).wait();
  txHashes.rollDice = rollReceipt!.hash;
  requestInfo.rollRequestedBlock = rollRequestedBlock;

  const rollRequestedEvent = (rollReceipt?.logs ?? [])
    .map((log) => {
      try {
    ...
    .find((parsed) => parsed?.name === "RollRequested");
  ...
  const lines = [
    ...
    "",
    `- [x] 8.5a ${walletFundingNote}` ,
    `- [x] 8.5b ${depositNote}`,
    "- [x] 8.5c openSession() moved the session into COME_OUT",
    `- [x] 8.5d placeBet(PASS_LINE, ${passLineBet}) updated the in-play/pass-line slot`,
    `- [x] 8.5e rollDice() entered ROLL_PENDING${requestInfo.requestId ? ` with requestId ${requestInfo.requestId}` : ""}`,
    "- [x] 8.5f Chainlink VRF fulfillment completed before timeout",
    `- [x] 8.5g Session advanced to phase ${resolvedState.phase.toString()} and pendingRequestId cleared`,
    `- [x] 8.5h withdraw(${withdrawAmount}) returned tokens to the wallet`,
  ...
  writeJson(path.join("deployments", "last-smoke-summary.json"), {
  ...
  console.log(`Smoke test completed for ${deployment.contractAddress}`);
  if (reportPath !== undefined) {
