...
function resolveReportPath(explicitPath: string | undefined): string | undefined {
  ...
  if (network.name === "baseSepolia") {
    return path.join("deployments", "sepolia-findings-v3.md");
  }
...
async function main() {
  ...
  const deploymentPath = args.deployment ?? process.env.CRAPS_SMOKE_DEPLOYMENT_V3 ?? defaultDeploymentPath();
  const reportPath = resolveReportPath(args.report ?? process.env.CRAPS_SMOKE_REPORT_V3);
  ...
  const game = await ethers.getContractAt("CrapsGameV3", deployment.contractAddress, signer);
  ...
  if (initialState.paused) {
    throw new Error("CrapsGameV3 is paused; smoke test cannot proceed");
  }
  ...
  const lines = [
    "# BASE Sepolia V3 smoke test findings",
    "",
    ...
    "- [ ] Live session expiry still requires a 24h wait (or a fork-based follow-up against the deployed V3 address)",
    "",
    "## Transactions",
    "",
  ...
  writeJson(path.join("deployments", "last-smoke-summary-v3.json"), {
  ...
  console.log(`V3 smoke test completed for ${deployment.contractAddress}`);
  if (reportPath !== undefined) {
