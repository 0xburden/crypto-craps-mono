...
async function verifyCrapsGameV3OnBasescan(deployment: CrapsGameV3DeploymentArtifact) {
  const apiKey = process.env.BASESCAN_API_KEY;
  if (!apiKey) {
    throw new Error("BASESCAN_API_KEY must be set to verify CrapsGameV3 on Basescan");
  }

  const providerNetwork = await ethers.provider.getNetwork();
  const chainId = Number(providerNetwork.chainId);
  const buildInfo = loadBuildInfo(path.join("artifacts", "contracts", "CrapsGameV3.sol", "CrapsGameV3.dbg.json"));
  ...
  fullInput.settings = {
    ...
    libraries: {
      "contracts/libraries/PayoutMathV3.sol": {
        PayoutMathV3: deployment.payoutMathLibrary,
      },
      "contracts/libraries/RollResolutionV3.sol": {
        RollResolutionV3: deployment.rollResolutionLibrary,
      },
    },
  };

  const factory = await ethers.getContractFactory("CrapsGameV3", {
    libraries: {
      PayoutMathV3: deployment.payoutMathLibrary,
      RollResolutionV3: deployment.rollResolutionLibrary,
    },
  ...
  const submitBody = new URLSearchParams({
    ...
    contractname: "contracts/CrapsGameV3.sol:CrapsGameV3",
  ...
  if (submitJson.status !== "1") {
    ...
    if (submitText.includes("already verified")) {
      console.log(`Already verified CrapsGameV3: ${deployment.contractAddress}`);
      return;
    }
  ...
  for (let attempt = 1; attempt <= BASESCAN_STATUS_MAX_POLLS; attempt += 1) {
    ...
    if (statusJson.status === "1") {
      console.log(`Verified CrapsGameV3 at ${deployment.contractAddress}`);
...
async function main() {
  ...
  await verifyWithRetry("PayoutMathV3", {
    address: deployment.payoutMathLibrary,
    contract: "contracts/libraries/PayoutMathV3.sol:PayoutMathV3",
    constructorArguments: [],
  });

  await verifyWithRetry("RollResolutionV3", {
    address: deployment.rollResolutionLibrary,
    contract: "contracts/libraries/RollResolutionV3.sol:RollResolutionV3",
    constructorArguments: [],
  });

  await verifyCrapsGameV3OnBasescan(deployment);
}
