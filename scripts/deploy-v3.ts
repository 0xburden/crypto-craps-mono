...
async function main() {
  ...
  const paramsFile = args.params ?? process.env.CRAPS_V3_PARAMS_FILE ?? defaultParamsFile();
  const outFile = args.out ?? process.env.CRAPS_V3_EXPORT_OUT ?? defaultOutputFile();
  ...
  console.log(`Deploying V3 with ${deployer.address} on ${network.name} (${chainId})`);
  ...
  const payout = await payoutFactory.deploy({ ...feeOverrides, nonce: nextNonce++ });
  await payout.waitForDeployment();
  const payoutAddress = await payout.getAddress();
  console.log(`- PayoutMathV3: ${payoutAddress}`);
  ...
  console.log(`- RollResolutionV3: ${resolverAddress}`);

  const gameFactory = await ethers.getContractFactory("CrapsGameV3", {
    libraries: {
      PayoutMathV3: payoutAddress,
      RollResolutionV3: resolverAddress,
    },
  });
  ...
  console.log(`- CrapsGameV3: ${gameAddress}`);
  ...
  const artifact = {
    ...
    payoutMathLibrary: payoutAddress,
  ...
  console.log(`Wrote V3 deployment artifact to ${path.resolve(outFile)}`);
  console.log(`Set frontend env: VITE_BASE_SEPOLIA_GAME_ADDRESS_V3=${gameAddress}`);
  console.log(`Set runtime env: CRAPS_BASE_SEPOLIA_GAME_ADDRESS_V3=${gameAddress}`);
}
