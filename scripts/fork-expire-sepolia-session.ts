...
async function main() {
  ...
  const game = await ethers.getContractAt("CrapsGame", deployment.contractAddress, executor);
  const token = await ethers.getContractAt([
    "function balanceOf(address owner) view returns (uint256)"
  ], deployment.tokenAddress, executor);
  ...
  await network.provider.send("evm_increaseTime", [SESSION_TIMEOUT_SECONDS + 1]);
  ...
  await network.provider.request({ method: "hardhat_impersonateAccount", params: [player] });
  await network.provider.request({
    method: "hardhat_setBalance",
    params: [player, "0x3635C9ADC5DEA00000"]
  });

  const playerSigner = await ethers.getSigner(player);
  const playerGame = await ethers.getContractAt("CrapsGame", deployment.contractAddress, playerSigner);
  const playerToken = await ethers.getContractAt([
    "function balanceOf(address owner) view returns (uint256)"
  ], deployment.tokenAddress, playerSigner);
  ...
  await network.provider.request({ method: "hardhat_stopImpersonatingAccount", params: [player] });

  const lines = [
    "# BASE Sepolia fork expiry findings",
    "",
    `- Generated at: ${new Date().toISOString()}`,
    `- Fork network: ${network.name}`,
    ...
    "## Result",
    "",
    "- [x] Fork-based 24h session expiry path succeeded",
  ...
  console.log(`Fork-based expiry check passed for ${player}`);
