...
export const HeaderBar = ({ game, sessionRemainingSeconds }: HeaderBarProps) => {
  ...
  return (
    <header className="machine-header sticky top-0 z-20 border-b border-emerald-300/10 bg-felt-950/88 backdrop-blur-xl">
      <div className="mx-auto grid max-w-[1600px] gap-3 px-3 py-3 sm:px-4 lg:px-6 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.45fr)_auto] xl:items-center">
        ...
        <div className="flex min-w-0 flex-wrap items-center gap-2 xl:justify-end">
          ...
          <span className="status-pill bg-white/5 text-slate-200">{getPhaseLabel(state?.phase)}</span>
          <span className="status-pill bg-white/5 text-slate-200">{getPuckLabel(state)}</span>
